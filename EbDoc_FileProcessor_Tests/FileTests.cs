...
namespace EbDoc_FileProcessor_Tests
{
    ...
    public class FileTests
    {
        ...
        public void loadMetadata_test_no_APNO()
        {
            ...
            //ACTION
            int actual_file_count = FileProcessor.loadMetadata(mockDB_Context, metadataFilePath);
        ...
        [TestMethod]
        public void create_archive_from_7zip()
        {
            string local_folder = System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location);
            string fileName = @"Test_Docs\TextFile1.txt";
            string archiveFile = @"Test_Docs\TextFile1.zip";
            string testfile =
                System.IO.Path.Combine(local_folder, fileName);

            string target = System.IO.Path.Combine(local_folder, archiveFile);

            FileProcessor.archive7zip(testfile, target);

            FileProcessor.archive7zip(System.IO.Path.Combine(local_folder, @"Test_Docs\TRADE_LICENSE_APPLICATIONS.txt"
                )
